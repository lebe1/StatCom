...
compute_rho <- function(n) {
  ...
  if (n == 1) {
    return(0)  # ρ1 is defined as 0
  }
  
  # Compute the logarithmic form of the ratio
  log_rho <- lgamma((n - 1) / 2) - lgamma(1/2) - lgamma((n - 2) / 2)
  
  # Convert the result back to the regular scale
  rho <- exp(log_rho)
...
print(rho_2000)
# Explanation: For large values of n, the gamma function overflows due to its computational limitations.
...
compute_rho <- function(n) {
  if (n %% 2 != 0) {
    ...
    stop("n must be an odd positive integer.")
  }
}
...
>>>>>>> 9423302f9a084b614d1780d44ec3de00bd7fb43a
...
=======
```{r}
kwtest_mult_fast <- function(X, grp) {
  res <- apply(X, 1, function(x) kwtest_fast(x, grp))
  
  return(res)
}

kwtest_mult_fast(X, grp)
```
...
As shown from the statistics, the loop approach is faster than the apply approach.
...
```
Since the Vectorize() uses mapply() under the hood, we can use this function right away to compare the results with the other two approaches. As shown, the loop approach is still the best.